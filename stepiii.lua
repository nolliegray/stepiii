...
local font = {
 ...
 ["A"]="011101111101101", ["L"]="100100100100111"
}
...
local function note_on(trk, vel)
 local ch = tracks[trk].channel
 if ch == 0 then for c = 1, 16 do midi_tx(143 + c, tracks[trk].note, vel or default_velocity) end
 else midi_tx(143 + ch, tracks[trk].note, vel or default_velocity) end
...
local function note_off(trk)
 ...
 local ch = tracks[trk].channel
 if ch == 0 then for c = 1, 16 do midi_tx(127 + c, tracks[trk].note, 0) end
 else midi_tx(127 + ch, tracks[trk].note, 0) end
...
local sys_m = metro.init(function()
 ...
 play_btn_cleared, play_btn_blink_time = true, sys_time; draw()
 end
 if play_btn_blink_time > 0 and (sys_time - play_btn_blink_time) >= 0.75 then play_btn_blink_time = 0; draw() end

 for py = 1, #tracks do
 local held = preset_held[py]
 if held.x and not held.saved and (sys_time - held.time) >= 2.0 then
 local px = held.x
 local function save_trk(trk, slot)
 presets[trk][slot] = { steps = {}, ratchets = {}, length = tracks[trk].length, muted = tracks[trk].muted, note = tracks[trk].note, channel = tracks[trk].channel, swing = tracks[trk].swing, micro_timing = tracks[trk].micro_timing }
 for i = 1, 32 do presets[trk][slot].steps[i], presets[trk][slot].ratchets[i] = tracks[trk].steps[i], tracks[trk].ratchets[i] end
 active_preset[trk], preset_blink[trk] = slot, {x = slot, time = sys_time}
 save_preset(trk, slot)
 end
 
 if held.col then for i=1, #tracks do save_trk(i, px) end else save_trk(py, px) end
 
 held.saved = true; active_preset.bpm = bpm; pending_global_save = true; draw()
 end
...
local ratchet_metro = metro.init(function()
 ...
 note_off(i); tracks[i].ratchet_vel = tracks[i].ratchet_vel or default_velocity; note_on(i, tracks[i].ratchet_vel)
 end
...
swing_metro = metro.init(function()
 ...
 local ch = t.channel
 if ch == 0 then for c=1,16 do midi_tx(143 + c, t.note, math.max(1, hit.step_val - 15)) end
 else midi_tx(143 + ch, t.note, math.max(1, hit.step_val - 15)) end
 table.insert(pending_swing_hits, {time = sys_time + hit.step_time / 2, track = hit.track, kind = "ratchet_redo"})
 else
 note_on(hit.track, hit.step_val)
 end
 t.playing_note = true
 elseif hit.kind == "ratchet_redo" then
 note_off(hit.track); note_on(hit.track, t.ratchet_vel or default_velocity)
 end
 end
...
local function advance_step()
 for i, t in ipairs(tracks) do if t.playing_note and not t.repeat_col and i ~= cycle_active_track then note_off(i) end end
 local max_len = 0
 ...
 t.ratchet_vel = val; 
 local ch = t.channel
 if ch == 0 then for c=1,16 do midi_tx(143 + c, t.note, math.max(1, val - 15)) end
 else midi_tx(143 + ch, t.note, math.max(1, val - 15)) end
 table.insert(pending_ratchet_hits, i); t.playing_note = true
 else note_on(i, val) end
 end
 end
 for i, t in ipairs(tracks) do
...
function event_grid(x, y, z)
 if x == 16 and y == 8 then
  if z == 1 then
   perform_btn_held = true
   perform_used_as_modifier = false
   perform_page_was_active = pages.perform
...
draw(); return 
 ...
 if z == 1 then 
  preset_held[y] = {x = x, time = sys_time, saved = false, col = perform_btn_held}
  if perform_btn_held then perform_used_as_modifier = true end
 elseif z == 0 and preset_held[y].x == x then
 if not preset_held[y].saved then
 local function load_trk(trk, slot)
 if tracks[trk].playing_note then note_off(trk) end
 local p = presets[trk][slot]
 if p then tracks[trk].length, tracks[trk].muted, tracks[trk].note, tracks[trk].channel, tracks[trk].swing, tracks[trk].micro_timing = p.length, p.muted or false, p.note or tracks[trk].note, p.channel or 10, p.swing or 50, p.micro_timing or 0
 for i=1,32 do tracks[trk].steps[i], tracks[trk].ratchets[i] = p.steps[i], p.ratchets[i] end
 else reset_track(tracks[trk]); tracks[trk].swing, tracks[trk].micro_timing = 50, 0 end
 active_preset[trk] = slot
 end
 
 if preset_held[y].col then for i = 1, #tracks do load_trk(i, x) end
 else load_trk(y, x) end
 
 active_preset.bpm = bpm; pending_global_save = true
 end
...
elseif x == 2 and y <= #tracks then selected_channel_track = y; draw()
 ...
 if x == 16 and y <= #tracks then 
 if perform_btn_held then 
  perform_used_as_modifier = true
  if tracks[y].muted then
   for i=1,#tracks do tracks[i].muted = false end
  else
   for i=1,#tracks do tracks[i].muted = true end
  end
 else 
...
grid_led(1, i, (not selected_channel_track and i == selected_track) and 15 or 4)
 ...
 if play_btn_blink_time > 0 and (sys_time - play_btn_blink_time) < 0.75 then play_br = ratchet_blink_state and 15 or 0 end
 grid_led(1, 8, play_br); grid_led(2, 8, 4); grid_led(3, 8, blink_state and 15 or (pages.bpm and 8 or 4))
